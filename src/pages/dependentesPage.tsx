...
useEffect(() => {
    ...
    // Filter clients who have a titular with the same id as passed in the route
    const filteredClientes = fetchedClientes.filter(cliente => cliente.titular === Number(id));

    // Find the titular with the given id (this should be the parent)
    const titular = fetchedClientes.find(cliente => cliente.id === Number(id));

    if (titular) {
        setClienteTitular(titular);  // Set the titular client
    }

    setClientes(filteredClientes);  // Set the dependents
}, [id]);  // Dependency on `id` so the effect runs again when `id` changes
    ...
    return (
        <div>
            ...
            <div className="container mt-4">
                <h3>Dependentes do cliente {clienteTitular?.nome}</h3>
