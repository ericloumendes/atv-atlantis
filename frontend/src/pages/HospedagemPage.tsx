...
useEffect(() => {
    const fetchData = async () => {
        ...
        if (fetchedAcomodacoes.length > 0) {
            setHospedagemFormData((prev) => ({
                ...prev,
                acomodacao: fetchedAcomodacoes[0],
            }));
        }

        const fetchedHospedagens = await fetchHospedagens();
...
const handleSaveAddHospedagem = async () => {
    const newHospedagem: Hospedagem = {
        ...hospedagemFormData,
        acomodacaoId: hospedagemFormData.acomodacao.id,
    };

    await addHospedagem(newHospedagem);

    const updatedList = await fetchHospedagens();
    setHospedagens(updatedList);
...
const handleSaveEditHospedagem = async () => {
    const updatedHospedagem = {
        ...hospedagemFormData,
        acomodacaoId: hospedagemFormData.acomodacao.id,
    };

    console.log("Submitting updated hospedagem:", updatedHospedagem); // ✅ See if triggered
    await editHospedagem(updatedHospedagem.id, updatedHospedagem);
...
const handleConfirmDeleteHospedagem = async () => {
    if (hospedagemToDelete !== null) {
        await removeHospedagem(hospedagemToDelete);
